...
- **R-8.2** While a challenge is active, `ForceCloseAlarmManager` maintains a rolling one-shot AlarmKit alarm ~20s out, rescheduled every 10s (new UUID each time, previous canceled only after the new one is registered — no gap). Force-closing the app therefore causes the alarm to re-fire. The current force-close ID is persisted in a shared per-parent backstop-slot dictionary. `StopIntent`, the app-side stale-slot sweep, and challenge cleanup only operate on the parent alarm's own slot. Completion retries a failed stop/cancel pair once; if it still fails, its slot is retained for the app-open sweep instead of being falsely cleared. A stale rolling registration restores the parent alert reference to the surviving registration's fire date after cancellation.
- **R-8.2a** Challenge-view disappearance is not evidence of completion and must never terminally cancel the backstop: on iOS 26.5 a force-quit tears down the scene (running `onDisappear`) before killing the process, and a terminal cancel there erased the protection (field bug 2026-07-22). `onDisappear` therefore only **suspends** the manager (stops the timer, blocks queued ticks via a suspended flag, bumps the generation so an in-flight schedule self-cancels through the stale-generation guard) while the current registration and its slot stay live. Terminal cancellation happens only in `complete()` → `stop()` before dismissal. Registrations surviving a suspend are reaped by R-8.3 orphan clear or the app-open sweep; if the process dies instead, the surviving registration re-fires (desired). Because `onDisappear` no longer double-checks the slot, the late-backstop-write race (a `StopIntent` slot write landing after a fast completion — the write has no cycle guard) is closed by a two-sided handshake instead: `StopIntent` re-reads pending-disarm after its slot write and revokes its own backstop when the cycle already ended, and `completeDisarmChallenge` revokes any slot survivor after final state is written (the manager's own registration was already stopped by `complete()`, so slot content there is by definition a late write). One side always observes the other's completed write; D-3 torn reads remain the accepted residual.
- **R-8.3** On challenge UI appear, an orphaned persisted force-close alarm for that parent from a previous crash/force-quit is adopted as the current registration until its replacement is successfully registered. Only then is the orphan canceled and the slot overwritten; if the first replacement schedule fails, the orphan and slot survive. Other parents' slots are left untouched.
