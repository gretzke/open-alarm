...
- **R-4.2** App (on notification or foreground) processes pending disarm IDs **one at a time**: resolves the parent alarm (the pending ID may be a bridge UUID), marks `lifecycleState = .awaitingDisarmChallenge`, resets `snoozeCount`, presents the disarm challenge UI with the alarm's resolved tasks. Pending disarm challenges are only presented while the app has a foreground-active scene and protected data is available; while locked or background-invisible the stop backstop loop keeps re-ringing. Unknown/stale pending IDs are dropped. An already-presented challenge is never replaced (avoids sound-manager races).
...
- **R-4.6** `StopIntent` uses one unified flow for normal alarms, bridge alarms, wake-check backup alarms, and force-close/backstop alarms. After parent resolution, alarms with tasks **or** wake-check enabled get a fresh 30s force-close/backstop alarm stored in a per-parent shared slot dictionary; scheduling retries once after `stop` + `cancel` of the fresh ID. The new slot is persisted before the previous per-parent backstop is stopped/canceled, so there is no gap. If both schedule attempts fail, the previous slot entry is left intact for app-side recovery. If protection is no longer needed, only that parent's slot is cleared. An unresolved intent ID is removed from pending-disarm and, only with a non-empty model, `StopIntent` attempts to cancel that ID. Resolution and the cancel guard evaluate the same model snapshot, so within that snapshot the ID is provably unowned (backstop intents carry the parent UUID, so an unresolved backstop targets at most the deleted parent's orphaned canonical slot); a D-3 torn read across processes remains the accepted residual risk. This is event-driven single-ID cleanup, not a sweep.
...
- **D-14** — **Deferred.** Override activation and force-reschedule persist bridge IDs, then await each bridge schedule. A disable that lands between suspension points can cancel every ID, then lose to the resumed schedule and re-register a just-cancelled bridge. Fixing this needs a bridge/canonical write-generation guard analogous to wake-check backup reconciliation.
