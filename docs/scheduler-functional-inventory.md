...
- **R-2.4** Enable/disable toggle:
  ...
  - Plain disable clears any override and cancels everything; plain enable resets `snoozeCount`, sets `lifecycleState = .scheduled`, schedules. During a wake-check session these operations persist the model only; disabling an override also cancels its bridge IDs without touching the backup slot (R-7.9).
