...
- **R-8.5** System volume is forced to the alarm level (currently 0.2) for the duration of the challenge: a hidden off-screen `MPVolumeView` slider sets the volume, a KVO observer on `outputVolume` reverts user changes, and a 0.2s polling timer catches held-down hardware volume buttons that KVO misses. The user cannot silence the challenge by turning the volume down. (Intentional anti-circumvention behavior — accepted App Store review risk; uses `MPVolumeView`'s internal slider.)
...
- **R-11.3** `SharedAlarmSettings` fields: snoozeEnabled, snoozeDurationMinutes, maxSnoozes (nil = unlimited), wakeUpCheckEnabled, wakeUpCheckDelayMinutes, wakeUpCheckResponseTimeoutMinutes, tasks. Wake-check minutes are clamped on every construction/decode path.
