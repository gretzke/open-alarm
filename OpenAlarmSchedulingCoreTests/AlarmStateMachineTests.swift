...
final class AlarmStateMachineTests: XCTestCase {
    ...
    func testPostRingReconciliationDeletesDueOneShotWhenAlarmKitNoLongerOwnsIt() throws {
        let calendar = Calendar(identifier: .gregorian)
        let createdAt = try XCTUnwrap(calendar.date(from: DateComponents(year: 2026, month: 5, day: 6, hour: 7, minute: 0, second: 10)))
        let now = try XCTUnwrap(calendar.date(from: DateComponents(year: 2026, month: 5, day: 6, hour: 7, minute: 2, second: 0)))
        var alarm = makeAlarm(hour: 7, minute: 1, deleteAfterUse: true)
        alarm.createdAt = createdAt
        alarm.updatedAt = createdAt

        let action = AlarmPostRingReconciler.action(
            for: alarm,
            liveAlarmKitIDs: [],
            pendingDisarmAlarmKitIDs: [],
            wakeCheckAlarmIDs: [],
            now: now,
            calendar: calendar
        )
    ...
    func testPostRingReconciliationKeepsDueOneShotWhenPendingDisarmOwnsIt() throws {
        let calendar = Calendar(identifier: .gregorian)
        let createdAt = try XCTUnwrap(calendar.date(from: DateComponents(year: 2026, month: 5, day: 6, hour: 7, minute: 0, second: 10)))
        let now = try XCTUnwrap(calendar.date(from: DateComponents(year: 2026, month: 5, day: 6, hour: 7, minute: 2, second: 0)))
        var alarm = makeAlarm(hour: 7, minute: 1, deleteAfterUse: true)
        ...
        let action = AlarmPostRingReconciler.action(
            for: alarm,
            liveAlarmKitIDs: [],
            pendingDisarmAlarmKitIDs: [alarm.id],
            wakeCheckAlarmIDs: [],
            now: now,
            calendar: calendar
        )
    ...
    func testPostRingReconciliationKeepsFutureOneShot() throws {
        let calendar = Calendar(identifier: .gregorian)
        let createdAt = try XCTUnwrap(calendar.date(from: DateComponents(year: 2026, month: 5, day: 6, hour: 7, minute: 0, second: 10)))
        let now = try XCTUnwrap(calendar.date(from: DateComponents(year: 2026, month: 5, day: 6, hour: 7, minute: 0, second: 30)))
        var alarm = makeAlarm(hour: 7, minute: 1, deleteAfterUse: true)
        alarm.createdAt = createdAt
        alarm.updatedAt = createdAt
    ...
    func testPostRingReconciliationCompletesDueKeptOneShot() throws {
        let calendar = Calendar(identifier: .gregorian)
        let createdAt = try XCTUnwrap(calendar.date(from: DateComponents(year: 2026, month: 5, day: 6, hour: 7, minute: 0, second: 10)))
        let now = try XCTUnwrap(calendar.date(from: DateComponents(year: 2026, month: 5, day: 6, hour: 7, minute: 2, second: 0)))
        var alarm = makeAlarm(hour: 7, minute: 1, deleteAfterUse: false)
        alarm.createdAt = createdAt
        alarm.updatedAt = createdAt
        ...
        XCTAssertEqual(action, .completeWithoutDeleting)
    }
}
