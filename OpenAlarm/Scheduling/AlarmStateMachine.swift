...
// MARK: - Post-ring reconciliation

enum AlarmPostRingReconciler {
    ...
    static func action(
        ...
        liveAlarmKitIDs: Set<UUID>,
        pendingDisarmAlarmKitIDs: Set<UUID>,
        wakeCheckAlarmIDs: Set<UUID>,
        now: Date = .now,
        calendar: Calendar = .autoupdatingCurrent
    ) -> Action {
        let alarmKitIDs = expectedAlarmKitIDs(for: alarm)

        guard liveAlarmKitIDs.isDisjoint(with: alarmKitIDs) else { return .keep }
        guard pendingDisarmAlarmKitIDs.isDisjoint(with: alarmKitIDs) else { return .keep }
        guard !wakeCheckAlarmIDs.contains(alarm.id) else { return .keep }

        switch alarm.lifecycleState {
        case .awaitingDisarmChallenge, .awaitingWakeCheck:
            return .keep
        case .scheduled, .alerting, .completed:
            break
        }

        guard alarm.activeOverride == nil else { return .keep }
        guard alarm.isEnabled else { return .keep }
        guard !alarm.isRepeating else { return .keep }
        guard hasScheduledFireTimeElapsed(for: alarm, now: now, calendar: calendar) else { return .keep }

        if alarm.deleteAfterUse || alarm.isNap || alarm.isTryOut {
            return .delete
        }

        return .completeWithoutDeleting
    }

    private static func expectedAlarmKitIDs(for alarm: AlarmDefinition) -> Set<UUID> {
    ...
    private static func hasScheduledFireTimeElapsed(
        for alarm: AlarmDefinition,
        now: Date,
        calendar: Calendar
    ) -> Bool {
        ...
        case .time(let hour, let minute):
            guard let fireDate = firstOneShotFireDate(
                hour: hour,
                minute: minute,
                scheduledAt: alarm.updatedAt,
                calendar: calendar
            ) else {
                return false
            }
    ...
    private static func firstOneShotFireDate(
        hour: Int,
        minute: Int,
        scheduledAt: Date,
        calendar: Calendar
    ) -> Date? {
        var components = calendar.dateComponents([.year, .month, .day], from: scheduledAt)
        ...
        // AlarmKit schedules by minute. If the user creates an alarm for the
        // current minute, treat that minute as the intended first occurrence;
        // otherwise roll past times to the next day.
        if sameDay.addingTimeInterval(60) <= scheduledAt {
